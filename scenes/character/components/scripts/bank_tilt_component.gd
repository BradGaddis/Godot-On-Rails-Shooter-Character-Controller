...
func _handle_flushing(delta: float):
	if vehicle_component.get_current_action() == ActorEnums.bank_tilt_actions.no_action:
		return
	if PlayerManager.character.visible_body.rotation.z != 0:
		vehicle_component.set_action(ActorEnums.bank_tilt_actions.flushing_rotation)
		...
		vehicle_component.set_action(ActorEnums.bank_tilt_actions.no_action)
...
func _handle_tilt(delta: float):
	...
	match vehicle_component.get_current_action():
		...
		_: # Any state that isn't tilting / banking
			...
			vehicle_component.set_action(ActorEnums.bank_tilt_actions.tilting)
