...
var launch: bool
var just_launched: bool = true;
## Emited when died
signal died

var velocity_death_epsilon: float = .05
#endregion
...
func update_physics_collision_layer(layer: int):
	collision_layer = layer

func update_physics_collision_mask(layer: int):
	collision_mask = layer

## Updates the colliision layer
func update_hitbox_collision_layer(layer: int):
	_hitbox.collision_layer = layer


## Updates the collision mask
func update_hitbox_collision_mask(layer: int):
...
func _physics_process(delta: float) -> void:
	...
	#apply_impulse(_launch_direction.normalized() * _shot_speed)
	add_constant_central_force(_launch_direction.normalized() * _shot_speed)
	_kill_self_on_distance()
	if just_launched and linear_velocity.length() < velocity_death_epsilon:
		return
	just_launched = false;
	#var collision = move_and_collide(_launch_direction * delta * (_shot_speed if not _tapered_velocity else _shot_speed * delta))
	#if collision:
		#queue_free()
	_kill_self_on_velocity_damped()


#func set_mass(amount: Curve) -> void:
	#mass = amount.sample()
...
func _kill_self_on_distance():
	if _initial_position.distance_to(position) >= _shot_range:
		queue_free()
		died.emit()



func _kill_self_on_time_out():
	pass


func _kill_self_on_velocity_damped(epsilon: float = .5):
