...
func _realign_rotation() -> Signal:
	...
	_vehicle_rotation_tween.tween_property(_player.character.vehicle_component, "rotation", Vector3.ZERO, .1)
...
func _loop() -> Signal:
	...
	_tween.tween_property(_player.character.vehicle_component.pivot_point, "rotation:x", TAU, 3)
...
func _player_entry_set_up() -> void:
	...
	_initial_flight_speed = _player.character.get_rail_speed()
...
func _resume_rail_speed_on_exit():
	...
	_out_tween.parallel().tween_property(_player.character, "_rail_speed", _initial_flight_speed, 1)
