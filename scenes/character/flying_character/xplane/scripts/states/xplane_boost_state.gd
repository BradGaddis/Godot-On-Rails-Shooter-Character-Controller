...
func enter(_last_state: String = ""):
	...
	_initial_fov = _player.character.camera_component.camera.fov
	_initial_flight_speed = _player.character.get_speed()
	_player.character.camera_component.tween_fov(_boosted_fov, _duration_time * 5)
	_active = true;
...
func exit(_next_state = ""):
	...
	_player.character.camera_component.tween_fov(_initial_fov, 1)
...
func _physics_process(_delta) -> void:
	...
	if _boost_exit_timer:
		...
		_player.character.set_speed(_initial_flight_speed + (_exit_flight_speed - _initial_flight_speed) * sampled_speed)
